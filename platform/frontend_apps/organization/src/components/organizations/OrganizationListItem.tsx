...
export function OrganizationListItem({ organization, isSelected = false, onClick }: OrganizationListItemProps) {
  return (
    <button
      onClick={onClick}
      className={cn(
        ...
        <div className="flex items-start space-x-3 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            {organization.description && (
              <p className="mt-1 text-sm text-gray-600 line-clamp-2">{organization.description}</p>
            )}
          </div>
        </div>
      </div>
