import { AppYamlConfig, AppYamlConfigSDK } from "@internal/app-yaml-static-config";
...
export async function mount(server) {
  server.get("/healthz/admin/app-yaml-static-config/status", async (request, reply) => {
    try {
      ...
      const providersConfig = instance.get("providers") || {};
      const providerTypes = {};
      for (const [name, config] of Object.entries(providersConfig)) {
        if (config && typeof config === "object" && config._type) {
          providerTypes[name] = config._type;
        }
      }
      return {
        initialized: true,
        app_env: instance.get("app_env") || "unknown",
        loaded_files: Array.from(instance.getOriginalAll().keys()),
