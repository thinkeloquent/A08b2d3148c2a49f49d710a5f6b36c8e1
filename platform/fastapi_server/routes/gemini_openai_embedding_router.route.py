"""
Gemini OpenAI Embedding Router Routes (Alias)

Alias endpoints mirroring gemini_openai_embedding.route.py under a different base path.
Base path: /api/llm/gemini-openai-embedding-v1-router/
...
BASE_PATH = "/api/llm/gemini-openai-embedding-v1-router"
...
def mount(app: FastAPI):
    ...
    async def _health_diagnostic(test_text: str = "Hello, this is a connection test.", model: str = "text-embedding-3-small"):
        ...
        try:
            ...
            return {
                "status": "error",
                "service": "gemini-openai-embedding-v1-router",
    ...
    async def llm_embedding_router_health(request: Request):
        """Embedding service health check (GET)."""
        return await _health_diagnostic()

    @app.post(f"{BASE_PATH}/health")
    async def llm_embedding_router_health_post(request: Request, body: HealthDiagnosticRequest):
    ...
    async def llm_router_embed(request: Request, body: EmbedRequest):
    ...
    async def llm_router_embed_query(request: Request, body: EmbedQueryRequest):
    ...
    async def llm_router_embed_batch(request: Request, body: EmbedRequest):
