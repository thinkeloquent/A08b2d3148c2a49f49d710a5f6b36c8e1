"""
Structured exception hierarchy for the Confluence API client.
...
class ConfluenceAPIError(Exception):
    ...
    def __init__(
        self,
        message: str = "Confluence API error",
...
class ConfluenceConflictError(ConfluenceAPIError):
    """409 - Conflict (e.g. version mismatch during update)."""
...
class ConfluenceServerError(ConfluenceAPIError):
    """5xx - Confluence server error."""

    def __init__(self, message: str = "Confluence server error", status_code: int = 500, **kwargs: Any) -> None:
        super().__init__(message, status_code=status_code, **kwargs)


class ConfluenceNetworkError(ConfluenceAPIError):
...
class ConfluenceTimeoutError(ConfluenceAPIError):
    """Request timed out."""
...
class ConfluenceConfigurationError(ConfluenceAPIError):
...
class SDKError(ConfluenceAPIError):
...
def create_error_from_response(
    ...
    if body:
        ...
        elif "statusCode" in body and "message" in body:
            detail = body["message"]
        else:
    ...
    if status == 400:
        return ConfluenceValidationError(detail, **kwargs)
    if status == 401:
        return ConfluenceAuthenticationError(detail, **kwargs)
    if status == 403:
        return ConfluencePermissionError(detail, **kwargs)
    if status == 404:
        return ConfluenceNotFoundError(detail, **kwargs)
    if status == 409:
        return ConfluenceConflictError(detail, **kwargs)
    if status == 429:
        return ConfluenceRateLimitError(detail, retry_after=retry_after, **kwargs)
    if status >= 500:
        return ConfluenceServerError(detail, status_code=status, **kwargs)
    return ConfluenceAPIError(detail, status_code=status, **kwargs)
