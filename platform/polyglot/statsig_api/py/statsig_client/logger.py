"""
Structured logger factory for the Statsig API client.
...
"""Regex pattern matching context-dict keys whose values should be redacted."""
...
def _redact_context(ctx: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    Keys that match ``_REDACT_KEYS`` anywhere in the key name are redacted.
    Nested dicts are redacted recursively.  ``None`` input passes through.
    """
    if ctx is None:
        return None
...
"""Mapping of level names to numeric severity."""


# ---------------------------------------------------------------------------
# Logger protocol (for type-checking custom logger objects)
# ---------------------------------------------------------------------------
...
def create_logger(package_name: str, filename: str) -> _Logger:
    ...
    ----------
    package_name:
        Logical package name (e.g. ``"statsig_client"``).
    filename:
        Source filename or module label (e.g. ``"client"``).

    Returns
    -------
    _Logger
        An object exposing ``debug``, ``info``, ``warning``, ``error`` methods.

    Environment
    -----------
    ``LOG_LEVEL``
        Controls minimum severity.  One of ``DEBUG``, ``INFO``, ``WARNING``,
        ``ERROR``, ``SILENT``.  Defaults to ``INFO`` when unset or invalid.
    """
