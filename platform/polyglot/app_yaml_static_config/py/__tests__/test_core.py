...
class TestAppYamlConfig:
    ...
    class TestDecisionBranchCoverage:
        ...
        def test_get_original_without_file_returns_none(self, base_config_path, fixtures_dir):
            """get_original() without file should return None, matching MJS undefined."""
            options = InitOptions(
                files=[base_config_path],
                config_dir=str(fixtures_dir)
            )
    ...
    class TestIntegration:
        ...
        def test_array_replace_during_merge(self, fixtures_dir):
            """Arrays should be fully replaced, not concatenated."""
            array_base = str(fixtures_dir / "array-base.yaml")
            array_override = str(fixtures_dir / "array-override.yaml")
            options = InitOptions(
                files=[array_base, array_override],
                config_dir=str(fixtures_dir)
            )
            AppYamlConfig.initialize(options)
            instance = AppYamlConfig.get_instance()

            tags = instance.get_nested("app", "tags")
            assert tags == ["gamma"]

            features = instance.get_nested("app", "settings", "features")
            assert features == ["feature-c"]

            name = instance.get_nested("app", "name")
            assert name == "test-app"

        def test_get_returns_deep_copy(self, base_config_path, fixtures_dir):
            """Mutations to get() return should not affect internal state."""
            options = InitOptions(
                files=[base_config_path],
                config_dir=str(fixtures_dir)
            )
            ...
            app_config = instance.get("app")
            app_config["name"] = "mutated"

            app_config2 = instance.get("app")
            assert app_config2["name"] == "test-app"

        def test_get_nested_returns_deep_copy(self, base_config_path, fixtures_dir):
            """Mutations to get_nested() return should not affect internal state."""
            options = InitOptions(
                files=[base_config_path],
                config_dir=str(fixtures_dir)
            )
            ...
            services = instance.get_nested("services")
            services["database"]["host"] = "mutated"

            services2 = instance.get_nested("services")
            assert services2["database"]["host"] == "localhost"

        def test_independent_state_after_reset(self, base_config_path, fixtures_dir):
            """Sequential initialize() after reset should produce independent state."""
            options1 = InitOptions(
                files=[base_config_path],
                config_dir=str(fixtures_dir)
            )
            AppYamlConfig.initialize(options1)
            config1 = AppYamlConfig.get_instance().get_all()

            AppYamlConfig._reset_for_testing()

            override_path = str(fixtures_dir / "override.yaml")
            options2 = InitOptions(
                files=[override_path],
                config_dir=str(fixtures_dir)
            )
            AppYamlConfig.initialize(options2)
            config2 = AppYamlConfig.get_instance().get_all()

            assert "name" not in config2.get("app", {})
            assert config2["app"]["environment"] == "production"
