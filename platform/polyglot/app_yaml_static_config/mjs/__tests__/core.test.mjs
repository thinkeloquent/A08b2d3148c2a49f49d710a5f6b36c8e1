...
describe('AppYamlConfig', () => {
    ...
    describe('Integration', () => {
        ...
        it('should replace arrays during merge (not concatenate)', async () => {
            const options = {
                files: [
                    path.join(FIXTURES_DIR, 'array-base.yaml'),
                    path.join(FIXTURES_DIR, 'array-override.yaml'),
                ],
                configDir: FIXTURES_DIR,
            };
            await AppYamlConfig.initialize(options);
            ...
            // Arrays should be fully replaced, not merged/concatenated
            const tags = instance.getNested(['app', 'tags']);
            expect(tags).toEqual(['gamma']);

            const features = instance.getNested(['app', 'settings', 'features']);
            expect(features).toEqual(['feature-c']);

            // Scalar values should be preserved from base
            const name = instance.getNested(['app', 'name']);
            expect(name).toBe('test-app');
        });

        it('should return deep copies from get() that do not affect internal state', async () => {
            const options = {
                files: [path.join(FIXTURES_DIR, 'base.yaml')],
                configDir: FIXTURES_DIR,
            };
            ...
            const appConfig = instance.get('app');
            appConfig.name = 'mutated';

            const appConfig2 = instance.get('app');
            expect(appConfig2.name).toBe('test-app');
        });

        it('should return deep copies from getNested() that do not affect internal state', async () => {
            const options = {
                files: [path.join(FIXTURES_DIR, 'base.yaml')],
                configDir: FIXTURES_DIR,
            };
            ...
            const services = instance.getNested(['services']);
            services.database.host = 'mutated';

            const services2 = instance.getNested(['services']);
            expect(services2.database.host).toBe('localhost');
        });

        it('should produce independent state after reset and re-initialize', async () => {
            const options1 = {
                files: [path.join(FIXTURES_DIR, 'base.yaml')],
                configDir: FIXTURES_DIR,
            };
            await AppYamlConfig.initialize(options1);
            const config1 = AppYamlConfig.getInstance().getAll();

            AppYamlConfig._resetForTesting();

            const options2 = {
                files: [path.join(FIXTURES_DIR, 'override.yaml')],
                configDir: FIXTURES_DIR,
            };
            await AppYamlConfig.initialize(options2);
            const config2 = AppYamlConfig.getInstance().getAll();

            // Second init should not contain base values
            expect(config2.app.name).toBeUndefined();
            expect(config2.app.environment).toBe('production');
        });
    });
});
