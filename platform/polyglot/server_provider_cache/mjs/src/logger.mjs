/**
 * Logger module for server-provider-cache package.
 *
 ...
 *     const logger = createLogger("server-provider-cache", "factory.mjs");
 *     logger.info("Factory initialized");
 *     const child = logger.child("providers");
...
const LOG_LEVEL = parseInt(process.env.CACHE_LOG_LEVEL || String(DEFAULT_LEVEL), 10);
...
/**
 * Logger class with package/filename context and child logger support
 */
class Logger {
    #package;
    #filename;
     ...
     * @param {string|null} instance - Instance name for child loggers
     * @param {number} level - Log level threshold
     * @param {Function} output - Output function (default: console.log)
     */
    constructor(packageName, filename, instance = null, level = LOG_LEVEL, output = console.log) {
     ...
     * @param {string} instanceName - Instance identifier (e.g., "providers", "config")
     * @returns {Logger} Child logger with instance prefix
     */
    child(instanceName) {
        return new Logger(
            this.#package,
 ...
 *
 * @param {string} packageName - Package identifier (e.g., "server-provider-cache")
 * @param {string} filename - Source file identifier (e.g., import.meta.url or "factory.mjs")
 ...
 *
 * @example
 * const logger = create("server-provider-cache", import.meta.url);
