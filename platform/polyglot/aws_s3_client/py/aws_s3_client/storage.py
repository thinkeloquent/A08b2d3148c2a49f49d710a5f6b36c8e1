...
class JsonS3Storage:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            custom_logger: Custom logger instance (default: package logger)
        """
        if not bucket_name:
    ...
    async def save(
        ...
        *,
        ...
        Returns:
            Storage key (custom or generated)
        ...
        if custom_key:
            key = custom_key
            self._logger.debug(f"save: using custom key={key}")
        else:
            key = generate_key(data, self._hash_keys)
            self._logger.debug(f"save: generating key for data with {len(data)} fields")
            self._logger.debug(f"save: generated key={key}")
        ...
        try:
            ...
            raise JsonS3StorageSerializationError(
                ...
                ErrorContext(operation="save", key=key, s3_key=s3_key),
            ) from e
    ...
    async def load(
        ...
        *,
        ...
        Args:
            data_or_key: Either data dict (for key generation) or key string
        ...
        if isinstance(data_or_key, dict):
            key = generate_key(data_or_key, self._hash_keys)
            self._logger.debug(f"load: generated key={key} from data")
        else:
            key = data_or_key
            self._logger.debug(f"load: using provided key={key}")
        ...
        try:
            ...
            if "accessdenied" in error_str or "403" in error_str:
                raise JsonS3StorageAuthError(
                    ...
                    ErrorContext(operation="load", key=key, s3_key=s3_key),
                ) from e
    ...
    async def delete(self, data_or_key: dict[str, Any] | str) -> bool:
        ...
        if isinstance(data_or_key, dict):
            key = generate_key(data_or_key, self._hash_keys)
            self._logger.debug(f"delete: generated key={key} from data")
        else:
            key = data_or_key
            self._logger.debug(f"delete: using provided key={key}")
        ...
        try:
            ...
            raise JsonS3StorageWriteError(
                ...
                ErrorContext(operation="delete", key=key, s3_key=s3_key),
            ) from e
    ...
    async def exists(self, data_or_key: dict[str, Any] | str) -> bool:
        ...
        try:
            ...
            raise JsonS3StorageReadError(
                ...
                ErrorContext(operation="exists", key=key, s3_key=s3_key),
            ) from e
    ...
    async def list_expired(self) -> list[str]:
        ...
        Returns:
            List of expired storage keys
        """
        self._check_closed("list_expired")

        self._logger.info("list_expired: scanning for expired entries")
        ...
        for key in keys:
            ...
            try:
                ...
                if entry.is_expired:
                    ...
                    self._logger.debug(f"list_expired: found expired key={key}")

            except Exception as e:
                self._logger.warn(f"list_expired: failed to check key={key}: {e}")
                continue

        self._logger.info(f"list_expired: found {len(expired_keys)} expired entries")
        return expired_keys
