...
class TestContextBuilder:
    ...
    class TestStatementCoverage:
        ...
        async def test_build_returns_context_dict(self):
            ...
            result = await ContextBuilder.build_static(options)
    ...
    class TestBranchCoverage:
        ...
        async def test_build_with_no_extenders(self):
            ...
            result = await ContextBuilder.build_static(options, extenders=None)
        ...
        async def test_build_with_empty_extenders_list(self):
            ...
            result = await ContextBuilder.build_static(options, extenders=[])
        ...
        async def test_build_with_single_extender(self):
            ...
            result = await ContextBuilder.build_static(options, extenders=[extender])
        ...
        async def test_build_with_multiple_extenders(self):
            ...
            result = await ContextBuilder.build_static(options, extenders=[extender1, extender2])
    ...
    class TestBoundaryValues:
        ...
        async def test_empty_options(self):
            ...
            result = await ContextBuilder.build_static({})
        ...
        async def test_empty_config(self):
            ...
            result = await ContextBuilder.build_static({"config": {}})
        ...
        async def test_none_request(self):
            ...
            result = await ContextBuilder.build_static({"request": None})
        ...
        async def test_deeply_nested_config(self):
            ...
            result = await ContextBuilder.build_static({"config": deep_config})
    ...
    class TestErrorHandling:
        ...
        async def test_extender_exception_propagates(self):
            """Exceptions in extenders should be caught and logged (defensive)."""
            async def bad_extender(ctx, req):
                raise ValueError("Extender error")

            options = {}
            # Should not raise
            await ContextBuilder.build_static(options, extenders=[bad_extender])

        @pytest.mark.asyncio
        async def test_extender_returning_none(self):
            """Extender returning None should be handled gracefully."""
            async def none_extender(ctx, req):
                return None

            options = {"config": {"key": "value"}}
            # Should not raise TypeError
            await ContextBuilder.build_static(options, extenders=[none_extender])
    ...
    class TestIntegration:
        ...
        async def test_full_context_build_with_request(self):
            ...
            result = await ContextBuilder.build_static(options, extenders=[auth_extender])
            ...
            assert result["request"] is mock_request
            assert result["auth"]["token"] == "bearer ****"
