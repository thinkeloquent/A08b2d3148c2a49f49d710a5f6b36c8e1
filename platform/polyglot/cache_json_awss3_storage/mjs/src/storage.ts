...
export class JsonS3Storage {
  ...
  constructor(options: JsonS3StorageOptions) {
    ...
    if (options.logger) {
      ...
      this.logger = createLogger("cache_json_awss3_storage", import.meta.url);
    }
   ...
   *
   * @param key - Storage key (use generateKey() to create from object)
   * @param data - Object of data to save
   * @param options - Optional save options (ttl)
   * @returns Storage key
   *
   * @example
   * ```typescript
   ...
   * const key = generateKey({ userId: 123, action: "login" });
  ...
  async save<T extends Record<string, unknown>>(
    ...
    this.logger.debug(`save: key=${key}, s3Key=${s3Key}`);
    ...
    this.logger.info(`save: completed key=${key} in ${elapsed.toFixed(1)}ms`);
   ...
   *
   * @param key - Storage key (use generateKey() to create from object)
   * @param options - Optional load options (ignoreExpiry)
   * @returns Stored data object or null if not found/expired
   *
   * @example
   * ```typescript
   * // With custom key
  ...
  async load<T extends Record<string, unknown>>(
    ...
    this.logger.debug(`load: key=${key}`);
    const s3Key = this.buildS3Key(key);
    ...
    // Check expiration
    const isExpired = entry.expires_at !== null && Date.now() / 1000 > entry.expires_at;

    if (!options?.ignoreExpiry && isExpired) {
      this.logger.debug(`load: entry expired, deleting key=${key}`);
      ...
      try {
        ...
        this.logger.warn(`load: failed to delete expired entry: ${e}`);
      }
    ...
    this.stats.hits++;
    const elapsed = performance.now() - startTime;
    this.logger.info(`load: hit key=${key} in ${elapsed.toFixed(1)}ms`);
   ...
   *
   * @param key - Storage key (use generateKey() to create from object)
   * @returns True (S3 DELETE is idempotent)
   */
  async delete(key: string): Promise<boolean> {
    ...
    this.stats.deletes++;
    const elapsed = performance.now() - startTime;
    this.logger.info(`delete: completed key=${key} in ${elapsed.toFixed(1)}ms`);
  ...
  async exists(key: string): Promise<boolean> {
    ...
    const s3Key = this.buildS3Key(key);
    this.logger.debug(`exists: checking key=${key}`);
    ...
    try {
      ...
      this.logger.debug(`exists: key=${key} exists`);
      return true;
    } catch (e) {
      const error = e as Error;
      const errorStr = String(error).toLowerCase();
      ...
      if (
        ...
        this.logger.debug(`exists: key=${key} does not exist`);
        return false;
      }
   ...
   *
   * @returns Array of storage keys (prefix stripped)
   */
  async listKeys(): Promise<string[]> {
    this.checkClosed("listKeys");

    this.logger.info(
      `listKeys: listing objects with prefix=${this.keyPrefix}`
    );
    ...
    this.logger.info(
      `listKeys: found ${keys.length} keys in ${pageCount} pages`
    );
    return keys;
  }
  ...
  async clear(): Promise<number> {
    ...
    if (keys.length === 0) {
      this.logger.info("clear: no objects to delete");
      return 0;
    }
    ...
    this.logger.info(`clear: deleted ${deletedCount} objects`);
    return deletedCount;
  }
   ...
   *
   * @returns Count of deleted expired entries
   */
  async cleanupExpired(): Promise<number> {
    this.checkClosed("cleanupExpired");

    this.logger.info("cleanupExpired: scanning for expired entries");
    ...
    for (const key of keys) {
      ...
      try {
        ...
        if (isExpired) {
          ...
          this.logger.debug(`cleanupExpired: found expired key=${key}`);
        }
      } catch (e) {
        this.logger.warn(`cleanupExpired: failed to check key=${key}: ${e}`);
        continue;
      }
    }

    if (expiredKeys.length === 0) {
      this.logger.info("cleanupExpired: no expired entries found");
      return 0;
    }
    ...
    for (let i = 0; i < expiredKeys.length; i += batchSize) {
      ...
      try {
        ...
        this.logger.warn(`cleanupExpired: failed to delete batch: ${e}`);
        continue;
      }
    }

    this.logger.info(
      `cleanupExpired: deleted ${deletedCount} expired entries`
    );
    return deletedCount;
  }
   ...
   *
   * Subsequent operations will throw JsonS3StorageClosedError.
   */
  async close(): Promise<void> {
    if (this.closed) {
      this.logger.debug("close: already closed");
      return;
    }

    this.closed = true;
    this.logger.info("close: storage closed");
  }
  ...
  clearErrors(): void {
    this.errors = [];
    this.logger.debug("clearErrors: error history cleared");
  }
  ...
  async debugInfo(): Promise<DebugInfo> {
    ...
    return {
      ...
      encryption: this.encryption
        ? encryptionToS3Params(this.encryption)
        : null,
      objectCount,
      stats: this.getStats(),
 ...
 * @returns Configured JsonS3Storage instance
 *
 * @example
 * ```typescript
 * const storage = createStorage({
