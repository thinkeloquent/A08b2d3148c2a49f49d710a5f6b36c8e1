...
import './styles.css';
import { PanelLeftSidebarMenu002 } from '../src';
import type { FilterSectionConfig, SidebarItem } from '../src';
import { useState } from 'react';

// --- Sample Data ---

const CATEGORY_FILTERS: FilterSectionConfig = {
  key: 'category',
  title: 'Category',
  options: [
    { id: 'requirements', label: 'Requirements', count: 5 },
    { id: 'design', label: 'Design', count: 4 },
    { id: 'specification', label: 'Specification', count: 5 },
    { id: 'operational', label: 'Operational', count: 4 },
  ],
};

const STATUS_FILTERS: FilterSectionConfig = {
  key: 'status',
  title: 'Status',
  options: [
...
const MINIMAL_FILTERS: FilterSectionConfig[] = [
  {
    key: 'status',
    title: 'Status',
    options: [
...
const EXAMPLES = [
  ...
  { name: 'Minimal (Simple Filters)', path: '/minimal' },
];
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-slate-100 via-slate-50 to-indigo-50/30 p-4">
      <PanelLeftSidebarMenu002
        title="Templates"
        filterSections={[CATEGORY_FILTERS, STATUS_FILTERS]}
        items={items}
        itemFilter={(item, selections) => {
          ...
          if (statuses.includes('starred') && !item.starred) return false;
          return true;
        }}
...
function MinimalExample() {
  return (
    <div className="flex items-center justify-center min-h-screen bg-slate-100 p-4">
      <PanelLeftSidebarMenu002
        title="Projects"
        filterSections={MINIMAL_FILTERS}
        items={MINIMAL_ITEMS}
        itemFilter={(item, selections) => {
          const statuses = selections.status ?? [];
          if (statuses.length === 0) return true;
          if (statuses.includes('active') && item.category === 'active') return true;
          if (statuses.includes('archived') && item.category === 'archived') return true;
          return false;
        }}
