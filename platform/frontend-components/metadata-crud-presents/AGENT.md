...
A full-featured Template Management Platform UI for managing template presets with token replacements, a browsable catalog, generated instances, audit logging, approval workflows, references (which apps use templates), and labels/tags. Uses a compound component pattern so consumers can use the full shell or individual sub-components.
...
| `ApprovalRequest` | TypeScript type | `src/types.ts` |
...
| `TemplateManagePresents.Approvals` | Approval queue |
...
| approvals | `ApprovalRequest[]` | No | Pending approval requests |
...
| onApprovalAction | `(id, action, comment?) => void` | No | Called on approval/rejection |
