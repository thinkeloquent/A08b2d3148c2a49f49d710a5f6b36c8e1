...
export const PROJECT_TYPE_CONFIGS: Record<ProjectType, ProjectTypeConfig> = {
  ...
  'fastify-admin-apps': {
    ...
    displayName: 'Fastify Admin Apps Suite',
    description: 'Full-stack app: Fastify backend + React frontend + Admin dashboard + Protobuf + Sequelize',
    targetDirectory: (rootDir, name) => `${rootDir}/fastify_apps/${name.replace(/-/g, '_')}`,
    templatePath: 'fastify-admin-apps/template',
  },
  polyglot: {
    type: 'polyglot',
    displayName: 'Polyglot Module',
    description: 'Dual-language module with parallel mjs/ and py/ implementations',
    targetDirectory: (rootDir, name) => `${rootDir}/polyglot/${name.replace(/-/g, '_')}`,
    templatePath: 'polyglot/template',
  },
};
