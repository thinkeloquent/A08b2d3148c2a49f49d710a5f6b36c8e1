...
export function createCreateCommand(): Command {
  const command = new Command('create')
    ...
    .option('--target-dir <path>', 'Override the output directory')
    ...
    .action(async (type?: string, name?: string, options?: { targetDir?: string; withFrontend?: boolean; port?: number }) => {
      ...
      // Validate type
      const validTypes: ProjectType[] = ['fastapi', 'fastify', 'fastify-admin-apps', 'frontend', 'react-component', 'ts-package', 'py-package', 'health-check-provider', 'polyglot'];
