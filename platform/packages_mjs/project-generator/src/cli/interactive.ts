...
export const PROJECT_TYPES: Array<{
  ...
  hint: string;
    ...
    { value: 'fastify-admin-apps', label: 'Fastify Admin Apps Suite', hint: 'Backend + Frontend + Admin + Protobuf + Sequelize' },
    { value: 'polyglot', label: 'Polyglot Module', hint: 'Dual mjs/ + py/ module' },
  ];
...
function getPostCreateCommands(
  ...
  switch (projectType) {
    ...
    case 'py-package':
      ...
      break;
    case 'fastify-admin-apps':
      commands.push('# Setup database');
      commands.push('cd sequelize && pnpm run reset && cd ..');
      commands.push('# Start backend (port 3000)');
      commands.push('cd backend && pnpm dev');
      commands.push('# In new terminal: Start frontend (port 5173)');
      commands.push('cd frontend && pnpm dev');
      commands.push('# In new terminal: Start admin (port 5174)');
      commands.push('cd frontend-admin && pnpm dev');
      break;
    case 'polyglot':
      commands.push('make install');
