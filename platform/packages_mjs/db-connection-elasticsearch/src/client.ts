...
import { ElasticsearchConfig, ElasticsearchConfigOptions } from "./config.js";
import { VENDOR_ELASTIC_CLOUD, VENDOR_DIGITAL_OCEAN } from "./constants.js";
...
export async function checkConnection(
    ...
    try {
        ...
        // Use any to handle both Elasticsearch and OpenSearch client types
        const response = await (client as any).info();
        // OpenSearch client returns {body, statusCode, headers, meta}; ES v8 returns body directly
        const info = (response && typeof response === 'object' && 'body' in response && 'statusCode' in response)
            ? response.body
            : response;
