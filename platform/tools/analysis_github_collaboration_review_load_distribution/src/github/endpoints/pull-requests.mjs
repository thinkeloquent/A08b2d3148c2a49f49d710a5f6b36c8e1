...
export async function fetchPullRequests(ctx) {
  ...
  if (!config.ignoreDateRange && config.start && config.end) {
    ...
    for (const range of timeRanges) {
      const authorQuery = `author:${config.searchUser} is:pull-request ${config.partitionBy}:${range.start}..${range.end}`;
      const userQuery = `user:${config.searchUser} is:pull-request ${config.partitionBy}:${range.start}..${range.end}`;
