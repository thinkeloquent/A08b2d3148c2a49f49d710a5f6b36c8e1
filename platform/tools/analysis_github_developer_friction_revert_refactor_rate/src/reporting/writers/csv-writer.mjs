...
/**
 * Write CSV report files — 5 CSV files for revert/refactor rate analysis.
...
export async function writeCsvReport(report, outputPath) {
  ...
  // 2. Classified PRs CSV
  if (report.classifiedPRs && report.classifiedPRs.length > 0) {
    const prsPath = outputPath.replace(".csv", "-classified-prs.csv");
    const headers = [
      "number",
      "title",
      "repository",
    ...
    report.classifiedPRs.forEach((entry) => {
  ...
  // 5. Most reworked PRs CSV
  if (report.analytics?.mostReworkedPRs?.length > 0) {
    const reworkPath = outputPath.replace(".csv", "-most-reworked.csv");
    const reworked = report.analytics.mostReworkedPRs;
    const reworkHeaders = Object.keys(reworked[0]);
    let csv = reworkHeaders.join(",") + "\n";
    reworked.forEach((entry) => {
      const values = reworkHeaders.map((h) => formatCsvValue(entry[h]));
      csv += values.join(",") + "\n";
    });
    await fs.writeFile(reworkPath, csv, "utf8");
  }
}
