...
import { RevertRefactorAnalyzer } from "../analysis/revert-refactor-analyzer.mjs";

export class RevertRefactorRate {
  constructor(options) {
    this.config = normalizeConfig(options);

    // Mutable state
    this.data = { pullRequests: [], revertPRs: [], repositories: [] };
    this.errors = [];
    this.apiCalls = [];
    ...
    const streamFilename =
      ...
      `revert-refactor-rate-${this.config.searchUser || "unknown"}`;
    this.stream = new StreamWriter({
      outputDir: this.config.outputDir,
      filename: streamFilename,
      toolName: "revert-refactor-rate",
      toolConfig: { ...this.config },
    });
    this.ctx.stream = this.stream;

    // Initialize analyzer
    this.analyzer = new RevertRefactorAnalyzer(this.config);
    ...
    this.log("RevertRefactorRate initialized");
  }
  ...
  displayInitialization() {
    this.output(chalk.blue.bold("Revert/Refactor Rate Analyzer"));
    ...
    const params = [
      ...
      ["Rework threshold", `${this.config.reworkThreshold} review round-trips`],
      ["Post-merge window", `${this.config.postMergeWindowHours} hours`],
      [
        "Total records limit",
    ...
    this.output(
      chalk.blue.bold("\nStarting revert/refactor rate analysis...\n")
    );
  }
  ...
  performAnalysis(data) {
    ...
    try {
      ...
      this.log(`Failed to analyze revert/refactor rate: ${error.message}`, "warn");
      return {
        error: error.message,
  ...
  async generateReport(data, analytics, repositories) {
    ...
    const baseFilename =
      this.config.filename ||
      `revert-refactor-rate-${this.config.searchUser}-${timestamp}`;
  ...
  displaySummary(report, outputPath) {
    ...
    this.output("");
    this.output(chalk.green.bold("Revert/refactor rate analysis completed!"));
    ...
    this.output(
      `Total Merged PRs       : ${chalk.green(report.summary.totalMergedPRs)}`
    );
    this.output(
      `Reverted PRs           : ${chalk.red(report.summary.revertedPRs)}`
    );
    this.output(
      `Reworked PRs           : ${chalk.yellow(report.summary.reworkedPRs)}`
    );
    this.output(
      `Clean PRs              : ${chalk.green(report.summary.cleanPRs)}`
    );
    this.output(
      `Revert Rate            : ${chalk.red(
        (report.summary.revertRate * 100).toFixed(1) + "%"
      )}`
    );
    this.output(
      `Rework Rate            : ${chalk.yellow(
        (report.summary.reworkRate * 100).toFixed(1) + "%"
      )}`
    );
    this.output(
      `Friction Rate          : ${chalk.red(
        (report.summary.frictionRate * 100).toFixed(1) + "%"
      )}`
    );
    this.output(
      `Friction Classification: ${chalk.green(report.summary.frictionClassification)}`
    );
    this.output(
      `Avg Review Round-Trips : ${chalk.green(report.summary.avgReviewRoundTrips)}`
    );
    this.output(
      `Avg Changes Requested  : ${chalk.green(report.summary.avgChangesRequested)}`
    );
    this.output(
  ...
  async run() {
    try {
      ...
      try {
        ...
        const errFilename =
          this.config.filename ||
          `revert-refactor-rate-${this.config.searchUser}`;
        writeFileSync(
          path.join(
            this.config.outputDir,
