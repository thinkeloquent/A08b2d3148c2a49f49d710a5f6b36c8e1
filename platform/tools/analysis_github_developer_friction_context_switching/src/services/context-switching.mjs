...
import { ContextSwitchingAnalyzer } from "../analysis/context-switching.mjs";
...
export class ContextSwitching {
  constructor(options) {
    ...
    // Mutable state
    this.data = { activities: [], repositories: [] };
    this.errors = [];
    this.apiCalls = [];
    ...
    const streamFilename =
      ...
      `context-switching-${this.config.searchUser || "unknown"}`;
    this.stream = new StreamWriter({
      outputDir: this.config.outputDir,
      filename: streamFilename,
      toolName: "context-switching",
      toolConfig: { ...this.config },
    });
    this.ctx.stream = this.stream;

    // Initialize analyzer
    this.analyzer = new ContextSwitchingAnalyzer(this.config);
    ...
    this.log("ContextSwitching initialized");
  }
  ...
  displayInitialization() {
    this.output(chalk.blue.bold("Context Switching Analyzer"));
    ...
    const params = [
      ...
      ["Include commits", this.config.includeCommits ? "Yes" : "No"],
      ["Include PRs", this.config.includePRs ? "Yes" : "No"],
      ["Include reviews", this.config.includeReviews ? "Yes" : "No"],
      ["Session gap", `${this.config.minSessionGapMinutes} min`],
      [
        "Total records limit",
    ...
    this.output(
      chalk.blue.bold("\nStarting context switching analysis...\n")
    );
  }
  ...
  performAnalysis(data) {
    ...
    try {
      const analytics = this.analyzer.analyze(data.activities, {
        log: this.log,
      });
      return analytics;
    } catch (error) {
      this.log(`Failed to analyze context switching: ${error.message}`, "warn");
      return {
        error: error.message,
  ...
  async generateReport(data, analytics, repositories) {
    ...
    const baseFilename =
      this.config.filename ||
      `context-switching-${this.config.searchUser}-${timestamp}`;
  ...
  displaySummary(report, outputPath) {
    ...
    this.output("");
    this.output(chalk.green.bold("Context switching analysis completed!"));

    this.output("");
    this.output(chalk.blue.bold("Final Report Summary"));
    this.output("\u2500".repeat(60));
    this.output(
      `Total Activities       : ${chalk.green(report.summary.totalActivities)}`
    );
    this.output(
      `Context Switches       : ${chalk.green(report.summary.totalContextSwitches)}`
    );
    this.output(
      `Switch Rate            : ${chalk.green(report.summary.contextSwitchRate)}`
    );
    this.output(
      `Focus Score            : ${chalk.green(report.summary.focusScore)}`
    );
    this.output(
      `Focus Classification   : ${chalk.green(report.summary.focusClassification)}`
    );
    this.output(
      `Unique Repositories    : ${chalk.green(report.summary.uniqueRepositories)}`
    );
    this.output(
      `Avg Session Length     : ${chalk.green(report.summary.avgFocusSessionLength + " activities")}`
    );
    this.output(
      `Avg Time Between Sw.   : ${chalk.green(
        report.summary.avgTimeBetweenSwitchesHours !== null
          ? report.summary.avgTimeBetweenSwitchesHours + " hours"
          : "N/A"
      )}`
    );
    this.output(
  ...
  async run() {
    try {
      ...
      try {
        ...
        const errFilename =
          this.config.filename ||
          `context-switching-${this.config.searchUser}`;
        writeFileSync(
          path.join(
            this.config.outputDir,
