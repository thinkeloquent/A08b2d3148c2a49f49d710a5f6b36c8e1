...
/**
 * Interactive CLI for Technical Debt Ratio analysis.
 *
 * Usage:
 *   node cli.mjs              — interactive wizard (prompts for all options)
 *   node cli.mjs --searchUser — skip to non-interactive mode (delegates to bin/technical_debt_ratio.mjs)
 */
...
import { TechnicalDebtRatio } from "./src/services/technical-debt-ratio.mjs";
...
async function runInteractive() {
  p.intro(chalk.cyan.bold("Technical Debt Ratio Analyzer"));
  ...
  summaryLines.push(
    `  Classification     ${chalk.green("Keyword matching (Feature/Fix/Refactor/Chore)")}`,
  ...
  const confirmed = bail(
    await p.confirm({
      message: `Run technical debt ratio analysis for ${chalk.bold(options.searchUser || options.org || options.repo)}?`,
    })
  );
  ...
  const analyzer = new TechnicalDebtRatio(validated);
  await analyzer.run();
}
