...
import { TechnicalDebtAnalyzer } from "../analysis/technical-debt-analyzer.mjs";

export class TechnicalDebtRatio {
  constructor(options) {
    ...
    const streamFilename =
      ...
      `technical-debt-ratio-${this.config.searchUser || "unknown"}`;
    this.stream = new StreamWriter({
      outputDir: this.config.outputDir,
      filename: streamFilename,
      toolName: "technical-debt-ratio",
    ...
    this.log("TechnicalDebtRatio initialized");
  }
  ...
  displayInitialization() {
    this.output(chalk.blue.bold("Technical Debt Ratio Analyzer"));
    ...
    const params = [
      ...
      ["Classification", "Keyword matching (Feature/Fix/Refactor/Chore)"],
    ...
    this.output(
      chalk.blue.bold("\nStarting technical debt ratio analysis...\n")
    );
  }
  ...
  performAnalysis(data) {
    ...
    try {
      ...
      this.log(`Failed to analyze technical debt: ${error.message}`, "warn");
      return {
        error: error.message,
  ...
  async generateReport(data, analytics, repositories) {
    ...
    const baseFilename =
      this.config.filename ||
      `technical-debt-ratio-${this.config.searchUser}-${timestamp}`;
  ...
  displaySummary(report, outputPath) {
    ...
    this.output("");
    this.output(chalk.green.bold("Technical debt ratio analysis completed!"));
    ...
    this.output(
      `Features               : ${chalk.green(report.summary.featureCount)} (${report.summary.featurePercentage}%)`
    );
    this.output(
      `Fixes                  : ${chalk[report.summary.fixPercentage > 30 ? "red" : report.summary.fixPercentage > 20 ? "yellow" : "green"](
        report.summary.fixCount
      )} (${report.summary.fixPercentage}%)`
    );
    this.output(
      `Refactors              : ${chalk[report.summary.refactorPercentage > 30 ? "red" : report.summary.refactorPercentage > 20 ? "yellow" : "green"](
        report.summary.refactorCount
      )} (${report.summary.refactorPercentage}%)`
    );
    this.output(
      `Chores                 : ${chalk.green(report.summary.choreCount)} (${report.summary.chorePercentage}%)`
    );
    this.output(
      `Debt Commits (Fix+Ref) : ${chalk[report.summary.debtPercentage > 50 ? "red" : report.summary.debtPercentage > 40 ? "yellow" : "green"](
        report.summary.debtCount
      )} (${report.summary.debtPercentage}%)`
    );
    this.output(
      `Debt Ratio             : ${chalk[report.summary.debtRatio >= 0.5 ? "red" : report.summary.debtRatio >= 0.4 ? "yellow" : "green"](
        report.summary.debtRatio.toFixed(2)
      )}`
    );
    this.output(
      `Health Classification  : ${chalk[report.summary.healthClassification === "critical" || report.summary.healthClassification === "concerning" ? "red" : report.summary.healthClassification === "moderate" ? "yellow" : "green"](
  ...
  async run() {
    try {
      ...
      try {
        ...
        const errFilename =
          this.config.filename ||
          `technical-debt-ratio-${this.config.searchUser}`;
        writeFileSync(
          path.join(
            this.config.outputDir,
