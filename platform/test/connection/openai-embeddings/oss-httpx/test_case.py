#!/usr/bin/env python3
"""
Connection test for OpenAI Embeddings API via raw open-source httpx library.
...
def _resolve_api_key(provider_cfg: dict) -> str | None:
    """Resolve API key from config (with template guard) or env fallback."""
...
async def _test_connection(provider_cfg: dict, conn_cfg: dict) -> dict:
    """Test OpenAI Embeddings API connection using raw httpx.AsyncClient."""
...
async def main() -> int:
    ...
    summary = {
        "service": "openai-embeddings-oss-httpx",
