#!/usr/bin/env python3
"""
Connection test for GitHub API via the internal fetch_httpx package.
...
async def _test_connection(provider_cfg: dict, conn_cfg: dict) -> dict:
    """Test GitHub API connection using fetch_httpx.AsyncClient directly."""
    token = (
    ...
    if not token:
        return {"skipped": True, "reason": "no GITHUB_TOKEN, GH_TOKEN, or api_key in config"}
    ...
    try:
        ...
        async with AsyncClient(
            ...
            return {
                ...
                "endpoint": "/rate_limit",
                "has_rate_limit": "limit" in rate,
                "rate_limit_remaining": rate.get("remaining"),
                "latency_ms": latency,
                "connection": {
