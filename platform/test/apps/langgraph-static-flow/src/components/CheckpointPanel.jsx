...
export default function CheckpointPanel() {
  ...
  const { current, previous, previousThreadIds } = useMemo(() => {
    const cur = [];
    const prev = [];
    const threadSet = new Set();
    for (const cp of filteredCheckpoints) {
      if (currentThreadId && cp.threadId === currentThreadId) {
        cur.push(cp);
      } else {
        prev.push(cp);
        threadSet.add(cp.threadId);
      }
    }
    return { current: cur, previous: prev, previousThreadIds: [...threadSet] };
  }, [filteredCheckpoints, currentThreadId]);
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            ...
            <span className="text-[10px] text-slate-400">({current.length})</span>
          </div>
          {current.length > 0 ? (
        ...
        {/* Previous sessions — clickable rows that open modal */}
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            ...
            <span className="text-[10px] text-slate-400">({previousThreadIds.length})</span>
          </div>
          {previousThreadIds.length > 0 ? (
            <div className="space-y-1">
              {previousThreadIds.map((tid) => (
                <PreviousSessionRow
                  key={tid}
                  threadId={tid}
