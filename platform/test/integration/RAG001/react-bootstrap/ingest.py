#!/usr/bin/env python3
"""
RAG Ingestion Script for React Bootstrap
Indexes React Bootstrap components into a ChromaDB vector store.

Source structure: Single-package repo with flat component layout
  - Primary: src/ (144 component and utility .tsx files)
  - Flat structure: src/Accordion.tsx, src/Alert.tsx, src/Button.tsx, etc.
  - No subdirectories per component
"""
import os
import glob
...
persist_directory = os.environ.get("PERSIST_DIRECTORY", "db_react_bootstrap")
    ...
    if selected_ext:
        ...
        try:
            ...
            for doc in docs:
                ...
                doc.metadata["library"] = "react-bootstrap"
                # Flat structure: component name = file stem
...
def load_documents(source_dir: str, ignored_files: List[str] = []) -> List[Document]:
    all_files = []
    ignore_dirs = {"node_modules", "dist", "build", ".git", "storybook-static", "__tests__", "coverage", "lib", "esm"}
    ...
    with Pool(processes=os.cpu_count()) as pool:
        results = []
        with tqdm(total=len(filtered_files), desc="Ingesting react-bootstrap", ncols=80) as pbar:
...
def process_documents(ignored_files: List[str] = []) -> List[Document]:
    print(f"Loading React Bootstrap components from {source_directory}")
...
def main():
    ...
    if does_vectorstore_exist(persist_directory):
        ...
        print("Creating new React Bootstrap vector store")
    ...
    print("Ingestion complete! You can now query the React Bootstrap library.")
