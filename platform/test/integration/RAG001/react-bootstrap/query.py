#!/usr/bin/env python3
"""
Interactive RAG Query Interface for React Bootstrap
Search components and ask LLM-powered questions against the indexed vector store.

Usage:
    python query.py
    PERSIST_DIRECTORY=db_react_bootstrap LLM_PROVIDER=anthropic python query.py

Environment variables:
    PERSIST_DIRECTORY   - ChromaDB path (default: db_react_bootstrap)
...
LIBRARY_NAME = "React Bootstrap"
LIBRARY_SLUG = "react-bootstrap"
...
about components, props, Bootstrap integration, and usage based on the source code provided as context.

Rules:
- Only answer based on the provided context. If the context doesn't contain enough information, say so.
- When referencing components, mention the file path (e.g., src/Button.tsx) so the user can find it.
- Note Bootstrap CSS class mappings and variant props when relevant.
