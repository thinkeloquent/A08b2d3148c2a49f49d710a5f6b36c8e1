#!/usr/bin/env python3
"""
Interactive RAG Query Interface for Material UI (MUI)
Search components and ask LLM-powered questions against the indexed vector store.

Usage:
    python query.py
    PERSIST_DIRECTORY=db_material_ui LLM_PROVIDER=anthropic python query.py

Environment variables:
    PERSIST_DIRECTORY   - ChromaDB path (default: db_material_ui)
...
def format_metadata(meta: dict) -> str:
    parts = []
    if "package" in meta:
        parts.append(f"pkg={meta['package']}")
...
SYSTEM_PROMPT = f"""You are an expert on the {LIBRARY_NAME} (MUI) component library. You answer questions
about components, props, theming (createTheme, sx prop, styled), and usage based on the source code provided as context.

Rules:
- Only answer based on the provided context. If the context doesn't contain enough information, say so.
- When referencing components, mention the MUI package (mui-material, mui-joy, mui-lab) and file path.
- Distinguish between Material Design (mui-material) and Joy UI (mui-joy) when relevant.
- Provide code examples when relevant, including sx prop and theme customization patterns.
