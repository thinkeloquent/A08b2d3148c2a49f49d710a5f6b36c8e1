#!/usr/bin/env python3
"""
RAG Ingestion Script for Material UI (MUI)
Indexes MUI components into a ChromaDB vector store.

Source structure: Large pnpm monorepo with 26 packages
  - Primary: packages/mui-material/src (158 component dirs, PascalCase)
  - Additional: packages/mui-joy, packages/mui-lab, packages/mui-system, etc.
"""
import os
import glob
...
persist_directory = os.environ.get("PERSIST_DIRECTORY", "db_material_ui")
# Index the full packages dir to capture mui-material, mui-joy, mui-lab, etc.
    ...
    if selected_ext:
        ...
        try:
            ...
            for doc in docs:
                ...
                doc.metadata["library"] = "material-ui"
                # Extract package name: packages/<name>/...
                parts = file_path.split(os.sep)
                if "packages" in parts:
                    idx = parts.index("packages")
                    if idx + 1 < len(parts):
                        doc.metadata["package"] = parts[idx + 1]
                # MUI uses PascalCase component directories under src/
                if "src" in parts:
                    idx = parts.index("src")
                    if idx + 1 < len(parts) and parts[idx + 1][0].isupper():
...
def load_documents(source_dir: str, ignored_files: List[str] = []) -> List[Document]:
    all_files = []
    ignore_dirs = {"node_modules", "dist", "build", ".git", "storybook-static", "__tests__", "coverage", "esm"}
    ...
    with Pool(processes=os.cpu_count()) as pool:
        results = []
        with tqdm(total=len(filtered_files), desc="Ingesting material-ui", ncols=80) as pbar:
...
def process_documents(ignored_files: List[str] = []) -> List[Document]:
    print(f"Loading Material UI components from {source_directory}")
...
def main():
    ...
    if does_vectorstore_exist(persist_directory):
        ...
        print("Creating new Material UI vector store")
    ...
    print("Ingestion complete! You can now query the Material UI library.")
