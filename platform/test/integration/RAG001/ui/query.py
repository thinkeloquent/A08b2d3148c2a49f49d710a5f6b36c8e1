#!/usr/bin/env python3
"""
Interactive RAG Query Interface for shadcn/ui
Search components and ask LLM-powered questions against the indexed vector store.

Usage:
    python query.py
    PERSIST_DIRECTORY=db_shadcn_ui LLM_PROVIDER=anthropic python query.py

Environment variables:
    PERSIST_DIRECTORY   - ChromaDB path (default: db_shadcn_ui)
...
def format_metadata(meta: dict) -> str:
    ...
    if "registry_section" in meta:
        parts.append(f"section={meta['registry_section']}")
...
SYSTEM_PROMPT = f"""You are an expert on the {LIBRARY_NAME} component system. You answer questions
about component templates, the registry system, CLI commands, Tailwind CSS patterns, and Radix UI
primitive usage based on the source code provided as context.

Rules:
- Only answer based on the provided context. If the context doesn't contain enough information, say so.
- When referencing components, mention whether it's a registry template or CLI code, and the file path.
- Note that shadcn/ui generates components into user projects — explain the template patterns.
- Highlight Tailwind CSS class patterns and Radix primitive composition when relevant.
