#!/usr/bin/env python3
"""
RAG Ingestion Script for shadcn/ui
Indexes shadcn/ui component registry and templates into a ChromaDB vector store.

Source structure: pnpm monorepo (CLI tool, not a traditional component library)
  - Primary: packages/shadcn/src/registry (component definitions and templates)
  - CLI commands: packages/shadcn/src/commands
  - Components are templates that get generated into user projects
"""
import os
import glob
...
persist_directory = os.environ.get("PERSIST_DIRECTORY", "db_shadcn_ui")
# shadcn/ui stores component templates in the registry
source_directory = os.environ.get("SOURCE_DIRECTORY", os.path.join(REPO_ROOT, "packages", "shadcn", "src"))
    ...
    if selected_ext:
        ...
        try:
            ...
            for doc in docs:
                ...
                doc.metadata["library"] = "shadcn-ui"
                # Identify if file is a registry component or CLI code
                if "/registry/" in file_path:
                    doc.metadata["type"] = "registry"
                    # Extract component name from registry path
                    parts = file_path.split(os.sep)
                    if "registry" in parts:
                        idx = parts.index("registry")
                        if idx + 1 < len(parts):
                            doc.metadata["registry_section"] = parts[idx + 1]
                elif "/commands/" in file_path:
                    doc.metadata["type"] = "cli"
                else:
                    doc.metadata["type"] = "util"
            return docs
...
def load_documents(source_dir: str, ignored_files: List[str] = []) -> List[Document]:
    ...
    with Pool(processes=os.cpu_count()) as pool:
        results = []
        with tqdm(total=len(filtered_files), desc="Ingesting shadcn/ui", ncols=80) as pbar:
...
def process_documents(ignored_files: List[str] = []) -> List[Document]:
    print(f"Loading shadcn/ui components from {source_directory}")
...
def main():
    ...
    if does_vectorstore_exist(persist_directory):
        ...
        print("Creating new shadcn/ui vector store")
    ...
    print("Ingestion complete! You can now query the shadcn/ui library.")
