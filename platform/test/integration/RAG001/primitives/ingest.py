#!/usr/bin/env python3
"""
RAG Ingestion Script for Radix Primitives
Indexes Radix UI primitive components into a ChromaDB vector store.

Source structure: pnpm monorepo with per-component packages
  - Primary: packages/react/ (59 component packages)
  - Each component is its own package: packages/react/accordion/src/accordion.tsx
  - Headless, unstyled primitives (accessibility-focused)
"""
import os
import glob
...
# Each primitive is a separate package under packages/react/
source_directory = os.environ.get("SOURCE_DIRECTORY", os.path.join(REPO_ROOT, "packages", "react"))
    ...
    if selected_ext:
        ...
        try:
            ...
            for doc in docs:
                ...
                doc.metadata["library"] = "radix-primitives"
                # Extract primitive name: react/<primitive>/...
                parts = file_path.split(os.sep)
                if "react" in parts:
                    idx = parts.index("react")
                    if idx + 1 < len(parts):
...
def load_documents(source_dir: str, ignored_files: List[str] = []) -> List[Document]:
    ...
    with Pool(processes=os.cpu_count()) as pool:
        results = []
        with tqdm(total=len(filtered_files), desc="Ingesting primitives", ncols=80) as pbar:
...
def process_documents(ignored_files: List[str] = []) -> List[Document]:
    print(f"Loading Radix Primitives from {source_directory}")
...
def main():
    ...
    if does_vectorstore_exist(persist_directory):
        ...
        print("Creating new Radix Primitives vector store")
    ...
    print("Ingestion complete! You can now query the Radix Primitives library.")
