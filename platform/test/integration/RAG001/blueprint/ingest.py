#!/usr/bin/env python3
"""
RAG Ingestion Script for Blueprint (Palantir)
Indexes Blueprint components into a ChromaDB vector store.

Source structure: Lerna monorepo with 25 packages
  - Primary: packages/core/src/components (47 component dirs)
  - Additional: packages/datetime, packages/select, packages/table, etc.
"""
import os
import glob
...
persist_directory = os.environ.get("PERSIST_DIRECTORY", "db_blueprint")
# Blueprint is a monorepo — index the full packages directory to capture all component packages
    ...
    if selected_ext:
        ...
        try:
            ...
            for doc in docs:
                ...
                doc.metadata["library"] = "blueprint"
                ...
                # Extract component name from components/<name>/...
...
def load_documents(source_dir: str, ignored_files: List[str] = []) -> List[Document]:
    all_files = []
    ignore_dirs = {"node_modules", "dist", "build", ".git", "storybook-static", "__tests__", "coverage", "lib"}
    ...
    with Pool(processes=os.cpu_count()) as pool:
        results = []
        with tqdm(total=len(filtered_files), desc="Ingesting blueprint", ncols=80) as pbar:
...
def process_documents(ignored_files: List[str] = []) -> List[Document]:
    print(f"Loading Blueprint components from {source_directory}")
...
def main():
    ...
    if does_vectorstore_exist(persist_directory):
        ...
        print("Creating new Blueprint vector store")
    ...
    print("Ingestion complete! You can now query the Blueprint library.")
