#!/usr/bin/env python3
"""
RAG Ingestion Script for Mantine
Indexes Mantine components into a ChromaDB vector store.

Source structure: yarn monorepo with scoped packages
  - Primary: packages/@mantine/core/src/components (103 component dirs)
  - Additional: @mantine/dates, @mantine/hooks, @mantine/form, @mantine/charts, etc.
  - 19 packages under @mantine scope
"""
import os
import glob
...
persist_directory = os.environ.get("PERSIST_DIRECTORY", "db_mantine")
# Index the scoped @mantine packages
source_directory = os.environ.get("SOURCE_DIRECTORY", os.path.join(REPO_ROOT, "packages", "@mantine"))
...
LOADER_MAPPING = {
    ...
    ".story.tsx": (TextLoader, {"encoding": "utf8"}),
    ...
    ".module.css": (TextLoader, {"encoding": "utf8"}),
}
    ...
    if selected_ext:
        ...
        try:
            ...
            for doc in docs:
                ...
                doc.metadata["library"] = "mantine"
                # Extract package: @mantine/<name>/...
                parts = file_path.split(os.sep)
                if "@mantine" in parts:
                    idx = parts.index("@mantine")
                    if idx + 1 < len(parts):
                        doc.metadata["package"] = f"@mantine/{parts[idx + 1]}"
...
def load_documents(source_dir: str, ignored_files: List[str] = []) -> List[Document]:
    all_files = []
    ignore_dirs = {"node_modules", "dist", "build", ".git", "storybook-static", "__tests__", "coverage", "esm", "cjs"}
    ...
    with Pool(processes=os.cpu_count()) as pool:
        results = []
        with tqdm(total=len(filtered_files), desc="Ingesting mantine", ncols=80) as pbar:
...
def process_documents(ignored_files: List[str] = []) -> List[Document]:
    print(f"Loading Mantine components from {source_directory}")
...
def main():
    ...
    if does_vectorstore_exist(persist_directory):
        ...
        print("Creating new Mantine vector store")
    ...
    print("Ingestion complete! You can now query the Mantine library.")
