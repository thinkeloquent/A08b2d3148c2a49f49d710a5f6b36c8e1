#!/usr/bin/env python3
"""
Interactive RAG Query Interface for Mantine
Search components and ask LLM-powered questions against the indexed vector store.

Usage:
    python query.py
    PERSIST_DIRECTORY=db_mantine LLM_PROVIDER=anthropic python query.py

Environment variables:
    PERSIST_DIRECTORY   - ChromaDB path (default: db_mantine)
...
LIBRARY_NAME = "Mantine"
LIBRARY_SLUG = "mantine"
...
about components, props, hooks, theming (CSS Modules), and usage based on the source code provided as context.

Rules:
- Only answer based on the provided context. If the context doesn't contain enough information, say so.
- When referencing components, mention the @mantine package and file path so the user can find it.
- Note which @mantine package a component belongs to (core, dates, form, charts, etc.).
