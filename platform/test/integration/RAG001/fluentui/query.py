#!/usr/bin/env python3
"""
Interactive RAG Query Interface for Fluent UI (Microsoft)
Search components and ask LLM-powered questions against the indexed vector store.

Usage:
    python query.py
    PERSIST_DIRECTORY=db_fluentui LLM_PROVIDER=anthropic python query.py

Environment variables:
    PERSIST_DIRECTORY   - ChromaDB path (default: db_fluentui)
...
LIBRARY_NAME = "Fluent UI"
LIBRARY_SLUG = "fluentui"
...
SYSTEM_PROMPT = f"""You are an expert on the {LIBRARY_NAME} component library by Microsoft. You answer questions
about components, props, styling (Griffel/makeStyles), tokens, and usage based on the source code provided as context.

Rules:
- Only answer based on the provided context. If the context doesn't contain enough information, say so.
- When referencing components, mention the package name (e.g., react-button) and file path.
- Note the distinction between v9 (react-components) and v8 (legacy) APIs when relevant.
