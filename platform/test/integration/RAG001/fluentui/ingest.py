#!/usr/bin/env python3
"""
RAG Ingestion Script for Fluent UI (Microsoft)
Indexes Fluent UI components into a ChromaDB vector store.

Source structure: Large yarn monorepo with 48 packages
  - Primary: packages/react-components/ (86 individual component packages)
  - Each component is its own package (react-accordion, react-button, etc.)
  - Legacy v8 components at packages/react/
"""
import os
import glob
...
persist_directory = os.environ.get("PERSIST_DIRECTORY", "db_fluentui")
# Index the react-components subtree which contains all v9 component packages
source_directory = os.environ.get("SOURCE_DIRECTORY", os.path.join(REPO_ROOT, "packages", "react-components"))
    ...
    if selected_ext:
        ...
        try:
            ...
            for doc in docs:
                ...
                doc.metadata["library"] = "fluentui"
                # Extract package name: react-components/<package>/...
                parts = file_path.split(os.sep)
                if "react-components" in parts:
                    idx = parts.index("react-components")
                    if idx + 1 < len(parts):
...
def load_documents(source_dir: str, ignored_files: List[str] = []) -> List[Document]:
    all_files = []
    ignore_dirs = {"node_modules", "dist", "build", ".git", "storybook-static", "__tests__", "coverage", "lib", "lib-commonjs"}
    ...
    with Pool(processes=os.cpu_count()) as pool:
        results = []
        with tqdm(total=len(filtered_files), desc="Ingesting fluentui", ncols=80) as pbar:
...
def process_documents(ignored_files: List[str] = []) -> List[Document]:
    print(f"Loading Fluent UI components from {source_directory}")
...
def main():
    ...
    if does_vectorstore_exist(persist_directory):
        ...
        print("Creating new Fluent UI vector store")
    ...
    print("Ingestion complete! You can now query the Fluent UI library.")
