#!/usr/bin/env python3
"""
RAG Ingestion Script for Semantic UI React
Indexes Semantic UI React components into a ChromaDB vector store.

Source structure: Single-package repo organized by semantic categories
  - src/addons     - Additional functionality components
  - src/collections - Breadcrumb, Form, Grid, Menu, Message, Table
  - src/elements   - Button, Container, Divider, Icon, Image, Input, Label, List, etc.
  - src/modules    - Complex components like Modal, Dropdown
...
persist_directory = os.environ.get("PERSIST_DIRECTORY", "db_semantic_ui_react")
source_directory = os.environ.get("SOURCE_DIRECTORY", os.path.join(REPO_ROOT, "src"))
embeddings_model_name = os.environ.get("EMBEDDINGS_MODEL_NAME", "all-MiniLM-L6-v2")

chunk_size = 1200
chunk_overlap = 150

# Semantic UI category names for metadata enrichment
SEMANTIC_CATEGORIES = {"addons", "collections", "elements", "modules", "views"}
    ...
    if selected_ext:
        ...
        try:
            ...
            for doc in docs:
                ...
                doc.metadata["library"] = "semantic-ui-react"
                # Extract category and component from path: src/<category>/<Component>/...
                parts = file_path.split(os.sep)
                if "src" in parts:
                    idx = parts.index("src")
                    if idx + 1 < len(parts) and parts[idx + 1] in SEMANTIC_CATEGORIES:
                        doc.metadata["category"] = parts[idx + 1]
                        if idx + 2 < len(parts):
                            doc.metadata["component"] = parts[idx + 2]
            return docs
...
def load_documents(source_dir: str, ignored_files: List[str] = []) -> List[Document]:
    ...
    with Pool(processes=os.cpu_count()) as pool:
        results = []
        with tqdm(total=len(filtered_files), desc="Ingesting semantic-ui-react", ncols=80) as pbar:
...
def process_documents(ignored_files: List[str] = []) -> List[Document]:
    print(f"Loading Semantic UI React components from {source_directory}")
...
def main():
    ...
    if does_vectorstore_exist(persist_directory):
        ...
        print("Creating new Semantic UI React vector store")
    ...
    print("Ingestion complete! You can now query the Semantic UI React library.")
