#!/usr/bin/env python3
"""
Interactive RAG Query Interface for Semantic UI React
Search components and ask LLM-powered questions against the indexed vector store.

Usage:
    python query.py
    PERSIST_DIRECTORY=db_semantic_ui_react LLM_PROVIDER=anthropic python query.py

Environment variables:
    PERSIST_DIRECTORY   - ChromaDB path (default: db_semantic_ui_react)
...
LIBRARY_NAME = "Semantic UI React"
LIBRARY_SLUG = "Semantic-UI-React"
...
about components, props, and usage based on the source code provided as context.

Rules:
- Only answer based on the provided context. If the context doesn't contain enough information, say so.
- When referencing components, mention the category (addons, collections, elements, modules, views) and file path.
- Note Semantic UI CSS class generation patterns when relevant.
