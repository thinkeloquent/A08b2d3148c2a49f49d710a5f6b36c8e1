#!/usr/bin/env python3
"""
Interactive RAG Query Interface for Chakra UI
Search components and ask LLM-powered questions against the indexed vector store.

Usage:
    python query.py
    PERSIST_DIRECTORY=db_chakra_ui LLM_PROVIDER=anthropic python query.py

Environment variables:
    PERSIST_DIRECTORY   - ChromaDB path (default: db_chakra_ui)
...
LIBRARY_NAME = "Chakra UI"
LIBRARY_SLUG = "chakra-ui"
...
about components, props, theming, style props, and usage based on the source code provided as context.

Rules:
- Only answer based on the provided context. If the context doesn't contain enough information, say so.
- When referencing components, mention the package and file path so the user can find it.
- Provide code examples when relevant, especially showing Chakra style props usage.
