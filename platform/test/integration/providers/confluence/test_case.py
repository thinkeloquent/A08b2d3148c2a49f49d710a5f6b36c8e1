#!/usr/bin/env python3
"""Direct package test for confluence provider auth config with connection check."""
...
def main() -> int:
    ...
    provider_config = app_cfg.get_nested("providers", "confluence", default={})
    ...
    summary = {
        ...
        "app_yaml_providers_confluence": provider_config,
        "resolved_config": {
            "base_url": provider_config.get("base_url"),
            "health_endpoint": provider_config.get("health_endpoint"),
        ...
        "note": "auth_config resolves confluence auth (basic_email_token) from AppYamlConfig + env/context.",
    }
    ...
    summary["app_yaml_providers_confluence"] = {k: ("*******" if k in _sensitive else v) for k, v in summary["app_yaml_providers_confluence"].items()}
