#!/usr/bin/env python3
"""Direct package test for figma provider auth config with connection check."""
...
def _test_connection(provider_config: dict, api_key: str | None) -> dict:
    health_endpoint = provider_config.get("health_endpoint")
    base_url = provider_config.get("base_url")
    if not health_endpoint or not api_key:
        return {"skipped": True, "reason": "no health_endpoint or api_key"}
    try:
...
def main() -> int:
    ...
    provider_config = app_cfg.get_nested("providers", "figma", default={})
    auth_options = build_sdk_auth_options(
        provider_config,
    ...
    connection_test = _test_connection(provider_config, api_key)
    ...
    summary = {
        ...
        "app_yaml_providers_figma": provider_config,
        "resolved_config": {
            "base_url": provider_config.get("base_url"),
            "health_endpoint": provider_config.get("health_endpoint"),
            "auth_type": auth_options.get("type") if auth_options else None,
            "header_name": auth_options.get("headerName") if auth_options else None,
            "has_api_key": api_key is not None,
        },
        "connection_test": connection_test,
        "uses_app_yaml_directly": True,
        "note": "auth_config resolves figma auth (custom_header X-Figma-Token) from AppYamlConfig + env/context.",
    }
    ...
    summary["app_yaml_providers_figma"] = {k: ("*******" if k in _sensitive else v) for k, v in summary["app_yaml_providers_figma"].items()}
