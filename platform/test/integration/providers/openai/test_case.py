#!/usr/bin/env python3
"""Direct package test for openai provider auth config with connection check."""
...
def main() -> int:
    ...
    provider_config = app_cfg.get_nested("providers", "openai", default={})
    ...
    summary = {
        ...
        "app_yaml_providers_openai": provider_config,
        "resolved_config": {
            "base_url": provider_config.get("base_url"),
    ...
    summary["app_yaml_providers_openai"] = {k: ("*******" if k in _sensitive else v) for k, v in summary["app_yaml_providers_openai"].items()}
