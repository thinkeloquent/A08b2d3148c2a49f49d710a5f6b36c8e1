#!/usr/bin/env python3
"""Direct package test for anthropic provider auth config with connection check."""
...
def main() -> int:
    ...
    provider_config = app_cfg.get_nested("providers", "anthropic", default={})
    auth_options = build_sdk_auth_options(provider_config, default_auth_type="x-api-key")
    ...
    summary = {
        ...
        "app_yaml_providers_anthropic": provider_config,
        "resolved_config": {
            "base_url": provider_config.get("base_url"),
    ...
    summary["app_yaml_providers_anthropic"] = {k: ("*******" if k in _sensitive else v) for k, v in summary["app_yaml_providers_anthropic"].items()}
