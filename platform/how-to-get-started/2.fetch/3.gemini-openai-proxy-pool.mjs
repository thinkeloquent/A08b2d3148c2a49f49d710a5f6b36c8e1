/**
 * Gemini OpenAI-Compatible Proxy Pool Example
 *
 * Uses undici ProxyAgent for efficient connection pooling through a proxy.
 * Demonstrates singleton pattern for proxy connection management.
 *
 * Run: node 3.gemini-openai-proxy-pool.mjs
 *
...
function getProxyAgent() {
  if (!proxyAgentInstance) {
    ...
    if (PROXY_USER && PROXY_PASS) {
      options.token = 'Basic ' + Buffer.from(`${PROXY_USER}:${PROXY_PASS}`).toString('base64');
    }
...
async function main() {
  ...
  console.log('Gemini OpenAI-Compatible Proxy Pool Example');
  console.log('='.repeat(60));
  console.log();
  ...
  console.log(`Proxy Auth: ${PROXY_USER ? 'Yes' : 'No'}`);
  console.log();
