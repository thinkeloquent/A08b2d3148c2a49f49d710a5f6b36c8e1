/**
 * Tag Routes
 * CRUD endpoints for Figma file tags
 */
...
export default async function tagRoutes(fastify, _options) {
  ...
  fastify.get('/tags', {
    ...
    return reply.sendNegotiated(
      response,
      'figma_files.tag.TagListResponse'
    );
  });
  ...
  fastify.get('/tags/:id', {
    ...
    return reply.sendNegotiated(
      response,
      'figma_files.tag.GetTagResponse'
    );
  });
  ...
  fastify.post('/tags', {
    ...
    try {
      ...
      return reply.status(201).sendNegotiated(
        response,
        'figma_files.tag.CreateTagResponse'
      );
    } catch (error) {
  ...
  fastify.put('/tags/:id', {
    ...
    try {
      ...
      return reply.sendNegotiated(
        response,
        'figma_files.tag.UpdateTagResponse'
      );
    } catch (error) {
      if (error.name === 'SequelizeUniqueConstraintError') {
        return reply.status(409).send({
  ...
  fastify.delete('/tags/:id', {
    ...
    return reply.sendNegotiated(
      response,
      'figma_files.tag.DeleteTagResponse'
    );
  });
}
