...
import { useBulkCreateFigmaFiles } from '@/hooks';
import type { CreateFigmaFileRequest } from '@/types/api';
...
const exampleJson = `[
  {
    ...
    "figma_url": "https://www.figma.com/file/abc123/Design-System",
    "figma_file_key": "abc123XYZ",
    "status": "stable",
    "tag_names": ["design-system", "components"]
  }
]`;

interface ParseResult {
  valid: boolean;
  data?: CreateFigmaFileRequest[];
  error?: string;
  count?: number;
}
...
function parseJsonInput(input: string): ParseResult {
  ...
  try {
    ...
    const errors: string[] = [];
    const validTypes = ['design_system', 'component_library', 'prototype', 'illustration', 'icon_set'];
    ...
    parsed.forEach((item, index) => {
      ...
      if (!item.type || !validTypes.includes(item.type)) {
        errors.push(`Item ${index + 1}: missing or invalid "type" field (must be design_system, component_library, prototype, illustration, or icon_set)`);
      }
    });
...
export function BulkInsertPage() {
  ...
  const bulkCreate = useBulkCreateFigmaFiles();
      ...
      <div className="bg-white rounded-lg border border-gray-200">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-500 mt-1">
                Insert multiple Figma files at once using JSON array
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 space-y-6">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-medium">
                      {result.created > 0
                        ? `Successfully created ${result.created} figma file${result.created !== 1 ? 's' : ''}`
                        : 'No figma files created'}
                    </p>
              ...
              <table className="w-full">
                ...
                <tbody className="text-gray-700">
                  ...
                  <tr>
                    ...
                    <td className="py-1">"design_system" | "component_library" | "prototype" | "illustration" | "icon_set"</td>
                    <td className="py-1 text-red-600">Yes</td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    <td className="py-1 font-mono text-xs">figma_url</td>
                    <td className="py-1">string</td>
                    <td className="py-1">No</td>
                  </tr>
                  <tr>
                    <td className="py-1 font-mono text-xs">figma_file_key</td>
                    <td className="py-1">string</td>
                    <td className="py-1">No</td>
                  </tr>
                  <tr>
                    <td className="py-1 font-mono text-xs">thumbnail_url</td>
                    <td className="py-1">string</td>
                    <td className="py-1">No</td>
                  </tr>
                  <tr>
                    <td className="py-1 font-mono text-xs">page_count, component_count, style_count</td>
          ...
          <div className="flex items-center justify-end gap-4 pt-6 border-t border-gray-200">
            ...
            >
              <Upload className="w-4 h-4" />
              {bulkCreate.isPending ? 'Inserting...' : 'Insert Figma Files'}
            </button>
          </div>
        </div>
      </div>
