...
import { protoRoot } from '@internal/form-builder-proto';
 ...
 * e.g., "form_builder.form_definition.ListFormDefinitionsResponse"
 * Since protoRoot is the form_builder namespace itself,
 * we strip that prefix if present.
 */
function resolveProtoType(messagePath) {
  if (!messagePath) return null;

  // Strip "form_builder." prefix if present since protoRoot IS form_builder
  const normalizedPath = messagePath.startsWith('form_builder.')
    ? messagePath.slice('form_builder.'.length)
    : messagePath;
