/**
 * Tag Routes
 * CRUD endpoints for form definition tags
 */
...
export default async function tagRoutes(fastify, _options) {
  ...
  fastify.get('/tags', {
    ...
    return reply.sendNegotiated(
      response,
      'form_builder.tag.TagListResponse'
    );
  });
  ...
  fastify.get('/tags/:id', {
    ...
    return reply.sendNegotiated(
      response,
      'form_builder.tag.GetTagResponse'
    );
  });
  ...
  fastify.post('/tags', {
    ...
    try {
      ...
      return reply.status(201).sendNegotiated(
        response,
        'form_builder.tag.CreateTagResponse'
      );
    } catch (error) {
  ...
  fastify.put('/tags/:id', {
    ...
    try {
      ...
      return reply.sendNegotiated(
        response,
        'form_builder.tag.UpdateTagResponse'
      );
    } catch (error) {
      if (error.name === 'SequelizeUniqueConstraintError') {
        return reply.status(409).send({
  ...
  fastify.delete('/tags/:id', {
    ...
    return reply.sendNegotiated(
      response,
      'form_builder.tag.DeleteTagResponse'
    );
  });
}
