/**
 * FQDP Management System - Fastify Plugin
...
/**
 * FQDP Management System Plugin
 * Registers API endpoints for managing the FQDP hierarchy
 *
 ...
 */
async function fqdpManagementSystemPlugin(fastify, _options) {
  fastify.log.info("→ Initializing FQDP Management System plugin...");
  ...
  const apiPrefix = _options.apiPrefix || "/~/api/fqdp_management_system";

  // Health check endpoint
  fastify.get(apiPrefix, async (_request, _reply) => {
    return {
      status: "ok",
      service: "fqdp-management-system",
      version: "1.0.0",
      timestamp: new Date().toISOString(),
      endpoints: {
        health: `GET ${apiPrefix}`,
        organizations: `GET ${apiPrefix}/organizations`,
        workspaces: `GET ${apiPrefix}/workspaces`,
        teams: `GET ${apiPrefix}/teams`,
        applications: `GET ${apiPrefix}/applications`,
        projects: `GET ${apiPrefix}/projects`,
        resources: `GET ${apiPrefix}/resources`,
        references: `GET ${apiPrefix}/references`,
      },
    };
  });
  ...
  fastify.log.info("✅ FQDP Management System plugin successfully loaded");
...
// so we wrap in an arrow to keep fqdpManagementSystemPlugin's encapsulation intact for direct use.
export default fastifyPlugin(
  (fastify, opts) => fqdpManagementSystemPlugin(fastify, opts),
  { name: "fqdp-management-system" },
);

// Export the plugin function for direct use (encapsulated — no skip-override)
export { fqdpManagementSystemPlugin };
