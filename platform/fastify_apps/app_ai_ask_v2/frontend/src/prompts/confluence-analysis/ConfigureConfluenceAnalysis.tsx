...
"Page Tree Depth & Structure",
"Comment & Mention Activity",
"Contributor Engagement",
"Orphaned Pages",
"Space Growth Trends",
"Label Coverage",
"Inline vs Page Comments"];


const scopeOptions = ["All Spaces", "Team Spaces Only", "Personal Spaces Only", "Selected Space"];

const ConfigureConfluenceAnalysis = () => {
  const [scope, setScope] = useState("All Spaces");
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm" ref={dropRef}>
      <h3 className="font-bold text-slate-800 text-sm mb-4 flex items-center gap-2">
        <ConfluenceIcon s={18} data-test-id="confluenceicon-01a7a7a6" />
        Confluence Analysis
      </h3>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="relative">
          <label className="text-xs text-slate-500 font-semibold mb-1.5 block">Scope</label>
          <button
            onClick={() => {setScopeOpen(!scopeOpen);setCatOpen(false);}}
            ...
            <span className={`text-slate-400 transition-transform ${scopeOpen ? "rotate-180" : ""}`}>{Icon.chevDown}</span>
          </button>
          {scopeOpen &&
          <div className="absolute top-full mt-1 left-0 right-0 bg-white border border-slate-200 rounded-xl shadow-xl z-50 overflow-hidden">
            ...
            <button
              ...
              s === scope ? "bg-slate-100 text-slate-800 font-semibold" : "text-slate-600 hover:bg-slate-50"}`
              }>
        ...
        <div className="col-span-2 relative">
          ...
          <button
            onClick={() => {setCatOpen(!catOpen);setScopeOpen(false);}}
            className="w-full flex items-center justify-between bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-700 hover:border-slate-300 transition-colors">
          ...
          <div className="absolute top-full mt-1 left-0 right-0 bg-white border border-slate-200 rounded-xl shadow-xl z-50 overflow-hidden max-h-48 overflow-y-auto">
            ...
            <button
              ...
              "border-slate-300"}`
                    ...
                    {selectedCategories.includes(cat) &&
                <svg viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3" className="w-3 h-3" data-test-id="svg-aa074232">
        ...
        <div className="col-span-2">
          <label className="text-xs text-slate-500 font-semibold mb-1.5 block">CQL Filter (optional)</label>
          <input
            value={cql}
            ...
            placeholder='e.g. space = ENG AND lastModified > now("-90d")' />
        ...
        <div className="col-span-2">
          ...
          <div className="flex flex-col gap-2">
            {([
            [includePersonal, setIncludePersonal, "Include personal spaces"],
            [includeArchived, setIncludeArchived, "Include archived pages"]] as
            ...
            <label key={label} className="flex items-center gap-2 cursor-pointer group">
                ...
                val ? "border-blue-500 bg-blue-500" : "border-slate-300 group-hover:border-slate-400"}`
                  ...
                  {val &&
                <svg viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3" className="w-3 h-3" data-test-id="svg-c2fed88a">
                      <polyline points="20 6 9 17 4 12" />
                    </svg>
                }
                </div>
      ...
      <button
        ...
        {submitted ? "Analysis Complete" : "Run Confluence Health Analysis"}
      </button>
    </div>);

};

export { ConfigureConfluenceAnalysis };