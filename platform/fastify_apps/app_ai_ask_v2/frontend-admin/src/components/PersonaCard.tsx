        ...
        {isExpanded && (
          <div className="mt-4 pt-4 border-t border-gray-200 space-y-3">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {persona.goals.map((goal) => (
                    <span key={goal} className="px-2 py-1 bg-blue-50 text-blue-700 rounded-lg text-xs">
                      {goal}
                    </span>
                  ))}
                </div>
            ...
            {persona.tools && persona.tools.length > 0 && (
              <div>
                <h4 className="text-xs font-semibold text-gray-600 uppercase tracking-wider mb-2">Tools</h4>
                <div className="flex flex-wrap gap-2">
                  {persona.tools.map((tool) => {
                    const Icon = TOOL_ICONS[tool] || Wrench;
                    return (
                      <span key={tool} className="inline-flex items-center gap-1 px-2 py-1 bg-green-50 text-green-700 rounded-lg text-xs">
            ...
            {persona.permitted_to && persona.permitted_to.length > 0 && (
              <div>
                <h4 className="text-xs font-semibold text-gray-600 uppercase tracking-wider mb-2">Permitted To</h4>
                <div className="flex flex-wrap gap-2">
                  {persona.permitted_to.map((perm) => {
                    const Icon = PERMISSION_ICONS[perm] || Shield;
                    return (
                      <span key={perm} className="inline-flex items-center gap-1 px-2 py-1 bg-purple-50 text-purple-700 rounded-lg text-xs">
                        <Icon className="w-3 h-3" />
                        {perm}
                      </span>
                    );
