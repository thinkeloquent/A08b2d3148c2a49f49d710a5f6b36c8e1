...
export default function IterationTimeline() {
  const liveIterations = useGraphStore((s) => s.iterations);
  ...
  const graphActiveRunId = useGraphStore((s) => s.activeWorkflowRunId);
  const runStoreActiveRunId = useRunStore((s) => s.activeWorkflowRunId);
  const taskRuns = useRunStore((s) => s.taskRuns);
  const [expanded, setExpanded] = useState(null);

  // Determine which iterations to show:
  // - If the graph is currently running for the active run, use live iterations
  // - If viewing a historical run (different from live), convert task runs to iteration format
  // - Fallback: show live iterations
  const activeRunId = runStoreActiveRunId;
  const isLiveRun = activeRunId && activeRunId === graphActiveRunId && isRunning;
  const isHistoricalRun = activeRunId && activeRunId !== graphActiveRunId;

  const iterations = useMemo(() => {
    if (isHistoricalRun && taskRuns.length > 0) {
  ...
  }, [isHistoricalRun, taskRuns, liveIterations]);
