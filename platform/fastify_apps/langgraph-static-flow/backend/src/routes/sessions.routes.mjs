...
export default async function sessionRoutes(fastify, _options) {
  ...
  fastify.get('/sessions/current/:flowId', {
    ...
    if (!session) {
      return reply.status(404).send({ statusCode: 404, error: 'NotFound', message: `No active session found for flow "${flowId}"` });
    }
  ...
  fastify.get('/sessions/:id', {
    ...
    if (!session) {
      return reply.status(404).send({ statusCode: 404, error: 'NotFound', message: `Session with id "${id}" not found` });
    }
    return reply.send({ session: serializeSession(session) });
  });
  ...
  fastify.post('/sessions/:id/checkpoint', {
    schema: {
      ...
      body: { type: 'object', description: 'Checkpoint payload (arbitrary shape)' },
    },
  }, async (request, reply) => {
    const { id } = request.params;
  ...
  fastify.post('/sessions/:id/stage', {
    schema: {
      ...
      body: { type: 'object', description: 'Stage entry payload (arbitrary shape)' },
    },
  }, async (request, reply) => {
    const { id } = request.params;
