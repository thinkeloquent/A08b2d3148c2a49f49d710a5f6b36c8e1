...
import { protoRoot } from '@internal/code-repositories-proto';
 ...
 * e.g., "code_repositories.repository.ListRepositoriesResponse"
 * Since protoRoot is the code_repositories namespace itself,
 * we strip that prefix if present.
 */
function resolveProtoType(messagePath) {
  if (!messagePath) return null;

  // Strip "code_repositories." prefix if present since protoRoot IS code_repositories
  const normalizedPath = messagePath.startsWith('code_repositories.')
    ? messagePath.slice('code_repositories.'.length)
    : messagePath;
