...
/**
 * Hook to bulk create repositories
 */
export function useBulkCreateRepositories(
  options?: Omit<
    UseMutationOptions<BulkCreateResponse, ApiError, CreateRepositoryRequest[]>,
    'mutationFn'
  >
) {
  const queryClient = useQueryClient();
  ...
  return useMutation({
    mutationFn: (data: CreateRepositoryRequest[]) => repositoryApi.bulkCreate(data),
    onSuccess: () => {
      // Invalidate all repository lists to refetch with new data
      queryClient.invalidateQueries({ queryKey: repositoryKeys.lists() });
    },
