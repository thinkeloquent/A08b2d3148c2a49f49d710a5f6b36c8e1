...
async function databasePlugin(fastify, _options) {
  ...
  if (fastify.hasDecorator('db') && fastify.db?.sequelize) {
    fastify.log.info('Using Sequelize connection from lifecycle — adding rule_tree_table models');

    // Import and merge our models into the existing db object
    const { RuleTree, RuleItem } = await import('../../../sequelize/models/index.mjs');
    Object.assign(fastify.db, { RuleTree, RuleItem });
