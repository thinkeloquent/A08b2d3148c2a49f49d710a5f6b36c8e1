...
try {
  await fastify.register(fastifyStatic, {
    ...
    prefix: '/static/app/rule_tree_table/frontend/dist/',
  });
  fastify.log.info('✓ Static file serving registered for frontend');
} catch {
  fastify.log.warn('⚠ Frontend dist not found — skipping static file serving');
}
...
fastify.get('/', async (_request, reply) => {
  try {
    ...
    return reply.send({ message: 'Rule Tree Table API', docs: '/api/rule-tree-table' });
  }
});
