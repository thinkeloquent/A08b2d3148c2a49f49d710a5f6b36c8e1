...
export default async function stringTemplateRoutes(fastify, _options) {
  ...
  /**
   * Resolve a string template key.
   * NOTE: Declared before GET /string-templates/:id so the literal "resolve"
   * segment is not consumed by the :id param.
   * POST /string-templates/resolve
   */
  ...
  /**
   * Bulk upsert string templates.
   * NOTE: Declared before GET /string-templates/:id for the same reason.
   * POST /string-templates/bulk-upsert
   */
  ...
  fastify.get('/string-templates/:id', {
    ...
    if (!template) {
      return reply.status(404).send({
        statusCode: 404,
        error: 'NotFound',
        message: `String template with id "${id}" not found`,
      });
    }
  ...
  /**
   * Delete a string template.
   * DELETE /string-templates/:id
   */
