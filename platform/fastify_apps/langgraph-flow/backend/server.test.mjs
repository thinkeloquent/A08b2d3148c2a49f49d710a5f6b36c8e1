...
fastify.log.info('✓ LangGraph Flow plugin registered');
...
const start = async () => {
  try {
    ...
    fastify.log.info('Starting LangGraph Flow Server...');
    ...
    fastify.log.info(`✓ API: http://localhost:${port}/api/langgraph-flow`);
  } catch (err) {
    fastify.log.error({
