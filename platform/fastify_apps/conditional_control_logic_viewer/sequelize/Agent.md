# Conditional Control Logic Viewer Sequelize Database
...
export default function defineCodeRepository(sequelize) {
  return sequelize.define('CodeRepository', {
    ...
    tableName: 'conditional_control_logic_viewer_repositories',
    timestamps: true,
    underscored: true,
  });
}
...
export function initializeModels(sequelize) {
  ...
  CodeRepository.belongsToMany(Tag, {
    through: 'conditional_control_logic_viewer_repository_tags',
...
const sequelize = await getConnection({
  database: 'conditional_control_logic_viewer',
});
...
4. **Prefix table names**: With `conditional_control_logic_viewer_` for namespacing
