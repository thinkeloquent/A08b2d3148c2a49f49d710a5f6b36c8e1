...
fastify.log.info('✓ Conditional Control Logic Viewer plugin registered');
...
const start = async () => {
  try {
    ...
    fastify.log.info('🚀 Starting Conditional Control Logic Viewer Server...');
    ...
    fastify.log.info(`✓ API endpoint: http://localhost:${port}/api/conditional-control-logic-viewer`);
