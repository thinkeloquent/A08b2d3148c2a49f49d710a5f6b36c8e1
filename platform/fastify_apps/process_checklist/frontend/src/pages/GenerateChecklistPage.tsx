...
import { highlightPlaceholders } from "../utils/highlightParams";
...
export function GenerateChecklistPage() {
  ...
  const paramMap = Object.fromEntries(
    confirmedParameters.map((p) => [p.key, p.value])
  );
    ...
    <PageShell>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <Card
          ...
          description="Pick the blueprint you want to instantiate." data-test-id="card-882e6f3a">
        ...
        <Card
          ...
          description="Parameters let you inject client names, deadlines, environments, and any tokens your steps reference." data-test-id="card-6dc74b54">
          ...
          <Tabs
            ...
            },
            {
              ...
              <div className="space-y-4">
                    {!selectedTemplate ?
                <p className="text-sm italic text-neutral-500">
                        Select a template to preview replacement results.
                      </p> :
                !selectedTemplate.steps || selectedTemplate.steps.length === 0 ?
                <p className="text-sm italic text-neutral-500">
                        This template has no steps to preview.
                      </p> :

                <ol className="space-y-4">
                        {selectedTemplate.steps.map((step, index) =>
                  <li
                    key={step.stepId || `preview-${step.order}-${index}`}
                    className="rounded-lg border border-neutral-200 bg-white p-6 transition hover:border-emerald-300">
                              ...
                              <div className="flex-1 space-y-3">
                                <div className="flex flex-wrap items-center justify-between gap-3">
                                  <h3 className="text-lg font-semibold text-neutral-900">
                                    {highlightPlaceholders(step.title, paramMap)}
                                  </h3>
                                  {step.required ?
                          ...
                          <span className="inline-flex items-center gap-2 rounded-full bg-neutral-100 px-3 py-1 text-xs font-medium uppercase tracking-wide text-neutral-600">
                                ...
                                {step.description ?
                        <p className="text-sm text-neutral-600">
                                    {highlightPlaceholders(step.description, paramMap)}
                                  </p> :
                        null}
        ...
        <div className="flex flex-wrap gap-4" data-test-id="div-cb8037e0">
          <button
            type="submit"
