# Prompt Management System Sequelize Database
...
export default function defineCodeRepository(sequelize) {
  return sequelize.define('CodeRepository', {
    ...
    tableName: 'prompt_management_system_repositories',
    timestamps: true,
    underscored: true,
  });
}
...
export function initializeModels(sequelize) {
  ...
  CodeRepository.belongsToMany(Tag, {
    through: 'prompt_management_system_repository_tags',
...
const sequelize = await getConnection({
  database: 'prompt_management_system',
});
...
4. **Prefix table names**: With `prompt_management_system_` for namespacing
