...
async function teardown() {
  ...
  try {
    ...
    const enumTypes = [
      'enum_persona_editor_personas_role',
      'enum_persona_editor_personas_tone',
      'enum_persona_editor_llm_defaults_category',
      'enum_persona_editor_audit_logs_action',
    ];
    for (const enumType of enumTypes) {
      await sequelize.query(`DROP TYPE IF EXISTS "${enumType}";`).catch(() => {});
    }

    console.log('Teardown complete (persona_editor schema only).');
  } catch (error) {
